...
- **Security & Compliance Artifacts**:
  - `*.sbom.json` - Software Bill of Materials in SPDX format (generated by Syft)
  - `*.intoto.jsonl` - SLSA provenance attestations (in-toto format)
  - `*.sig` - Cryptographic signatures (when signing is enabled)
...
> **Note**: SBOM format is SPDX (not CycloneDX). Signing with Cosign is configured but currently commented out in `.goreleaser.yml`.
