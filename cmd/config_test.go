...
func TestConfigSet_Help(t *testing.T) {
	cmd := exec.Command("go", "run", "./portctl/main.go", "config", "set", "--help")
...
func TestList_Help(t *testing.T) {
	cmd := exec.Command("go", "run", "./portctl/main.go", "list", "--help")
	output, err := cmd.CombinedOutput()
	if err != nil {
...
func TestQuick_Help(t *testing.T) {
	cmd := exec.Command("go", "run", "./portctl/main.go", "quick", "--help")
	output, err := cmd.CombinedOutput()
	if err != nil {
...
func TestScan_Help(t *testing.T) {
	cmd := exec.Command("go", "run", "./portctl/main.go", "scan", "--help")
	output, err := cmd.CombinedOutput()
	if err != nil {
...
func TestKill_Help(t *testing.T) {
	cmd := exec.Command("go", "run", "./portctl/main.go", "kill", "--help")
	output, err := cmd.CombinedOutput()
	if err != nil {
...
func TestWatch_Help(t *testing.T) {
	cmd := exec.Command("go", "run", "./portctl/main.go", "watch", "--help")
	output, err := cmd.CombinedOutput()
	if err != nil {
...
func TestConfigSetAndGet(t *testing.T) {
	...
	setCmd := exec.Command("go", "run", "./portctl/main.go", "config", "set", "output.format", "json")
	setOut, setErr := setCmd.CombinedOutput()
	if setErr != nil {
		t.Fatalf("Failed to set config value: %v\nOutput: %s", setErr, setOut)
	}
	// Get the config value
	getCmd := exec.Command("go", "run", "./portctl/main.go", "config", "get", "output.format")
...
func TestConfigSetAndGet_OutputColors(t *testing.T) {
	setCmd := exec.Command("go", "run", "./portctl/main.go", "config", "set", "output.colors", "false")
	setOut, setErr := setCmd.CombinedOutput()
	if setErr != nil {
		t.Fatalf("Failed to set output.colors: %v\nOutput: %s", setErr, setOut)
	}
	getCmd := exec.Command("go", "run", "./portctl/main.go", "config", "get", "output.colors")
	getOut, getErr := getCmd.CombinedOutput()
	if getErr != nil {
...
func TestConfigSetAndGet_ScanTimeout(t *testing.T) {
	setCmd := exec.Command("go", "run", "./portctl/main.go", "config", "set", "scan.timeout", "30s")
	setOut, setErr := setCmd.CombinedOutput()
	if setErr != nil {
		t.Fatalf("Failed to set scan.timeout: %v\nOutput: %s", setErr, setOut)
	}
	getCmd := exec.Command("go", "run", "./portctl/main.go", "config", "get", "scan.timeout")
	getOut, getErr := getCmd.CombinedOutput()
	if getErr != nil {
...
func TestConfigSetAndGet_KillConfirm(t *testing.T) {
	setCmd := exec.Command("go", "run", "./portctl/main.go", "config", "set", "kill.confirm", "true")
	setOut, setErr := setCmd.CombinedOutput()
	if setErr != nil {
		t.Fatalf("Failed to set kill.confirm: %v\nOutput: %s", setErr, setOut)
	}
	getCmd := exec.Command("go", "run", "./portctl/main.go", "config", "get", "kill.confirm")
	getOut, getErr := getCmd.CombinedOutput()
	if getErr != nil {
...
func TestList_Exec(t *testing.T) {
	cmd := exec.Command("go", "run", "./portctl/main.go", "list")
	output, err := cmd.CombinedOutput()
...
func TestScan_Exec(t *testing.T) {
	cmd := exec.Command("go", "run", "./portctl/main.go", "scan", "127.0.0.1", "80")
	output, err := cmd.CombinedOutput()
...
func TestQuick_Exec(t *testing.T) {
	cmd := exec.Command("go", "run", "./portctl/main.go", "quick", "next-port")
	output, err := cmd.CombinedOutput()
